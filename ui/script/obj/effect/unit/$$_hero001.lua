-- ========================================================================
-- 【记录配置文件($$_hero001)】
-- 说明：单位所属的全部记录，除下面列出的记录外，单位记录树中还可以包含公共记录的根记录记录(common.xxx.xxx)。单位记录树所属记录只可以继承来自记录模板的记录记录（template.xxx）。特殊标记：1、#NIL代表强制赋值为nil，无视继承值。
-- ========================================================================

local user_data = require 'obj.effect.user_data'
local event_register = base.event_register
local mt = {
    ['ActorModel_1'] = {
        ['AutoScale'] = false,
        ['CreationFilter'] = {
            ['Ally'] = true,
            ['Enemy'] = true,
            ['Self'] = true,
        },
        ['EventCreation'] = 'on_cast_start',
        ['EventCreationModel'] = '',
        ['EventDestruction'] = 'on_cast_stop',
        ['EventDestructionModel'] = 'Death',
        ['FollowRotation'] = 1,
        ['ForceOneShot'] = 0,
        ['Model'] = '$$.unit.hero001.Model_2',
        ['Name'] = '模型表现节点（持续性）_1',
        ['Offset'] = {
            ['X'] = 0,
            ['Y'] = 120,
            ['Z'] = -30,
        },
        ['Rotation'] = {
            ['X'] = 90,
            ['Y'] = 120,
            ['Z'] = 0,
        },
        ['Scale'] = 1,
        ['ScaleByParent'] = 1,
        ['ShowShadow'] = true,
        ['SocketName'] = 'socket_back',
        ['UseRoleSH'] = false,
        ['sync'] = true,
        ['ID'] = 'ActorModel_1',
        ['Class'] = 'actor',
        ['NodeType'] = 'ActorModel',
        ['Template'] = 'ActorModel',
        ['Link'] = '$$.unit.hero001.ActorModel_1',
        ['user_data'] = user_data['$$.unit.hero001'],
        ['event'] = event_register
    },
    ['ItemContainer_1'] = {
        ['Name'] = '背包/装备栏节点_1',
        ['Slots'] = {
            [1] = {
                ['Equip'] = false,
                ['Excluded'] = {
                },
                ['GrantTag'] = '',
                ['Icon'] = '',
                ['Required'] = {
                },
                ['Socket'] = '',
            },
            [2] = {
                ['Equip'] = false,
                ['Excluded'] = {
                },
                ['GrantTag'] = '',
                ['Icon'] = '',
                ['Required'] = {
                },
                ['Socket'] = '',
            },
            [3] = {
                ['Equip'] = false,
                ['Excluded'] = {
                },
                ['GrantTag'] = '',
                ['Icon'] = '',
                ['Required'] = {
                },
                ['Socket'] = '',
            },
            [4] = {
                ['Equip'] = false,
                ['Excluded'] = {
                },
                ['GrantTag'] = '',
                ['Icon'] = '',
                ['Required'] = {
                },
                ['Socket'] = '',
            },
            [5] = {
                ['Equip'] = false,
                ['Excluded'] = {
                },
                ['GrantTag'] = '',
                ['Icon'] = '',
                ['Required'] = {
                },
                ['Socket'] = '',
            },
            [6] = {
                ['Equip'] = false,
                ['Excluded'] = {
                },
                ['GrantTag'] = '',
                ['Icon'] = '',
                ['Required'] = {
                },
                ['Socket'] = '',
            },
        },
        ['ID'] = 'ItemContainer_1',
        ['Class'] = 'item_container',
        ['NodeType'] = 'ItemContainer',
        ['Template'] = 'ItemContainer',
        ['Link'] = '$$.unit.hero001.ItemContainer_1',
        ['user_data'] = user_data['$$.unit.hero001'],
        ['event'] = event_register
    },
    ['Model_1'] = {
        ['ActorArray'] = nil,
        ['AnimMapping'] = {
            [1] = {
                ['NameFrom'] = 'Idle',
                ['NameTo'] = '0_idle2',
            },
            [2] = {
                ['NameFrom'] = 'Move',
                ['NameTo'] = 'bp_run',
            },
            [3] = {
                ['NameFrom'] = 'Spell',
                ['NameTo'] = 'skill_shieldattack',
            },
        },
        ['Asset'] = 'characters/general/sk_dress2/model.prefab',
        ['AutoScaleBaseRadius'] = 128,
        ['BirthStandDeathAnim'] = 'bsd',
        ['Events'] = {
        },
        ['ExtraAnimations'] = {
        },
        ['Name'] = '模型节点_1',
        ['Scale'] = 1,
        ['ID'] = 'Model_1',
        ['Class'] = 'model',
        ['NodeType'] = 'Model',
        ['Template'] = 'Model',
        ['Link'] = '$$.unit.hero001.Model_1',
        ['user_data'] = user_data['$$.unit.hero001'],
        ['event'] = event_register
    },
    ['Model_2'] = {
        ['ActorArray'] = nil,
        ['AnimMapping'] = {
        },
        ['Asset'] = 'eqpt/weapon/sk_wand/model.prefab',
        ['AutoScaleBaseRadius'] = 128,
        ['BirthStandDeathAnim'] = 'bsd',
        ['Events'] = {
        },
        ['ExtraAnimations'] = {
        },
        ['Name'] = '模型节点_2',
        ['Scale'] = 1,
        ['ID'] = 'Model_2',
        ['Class'] = 'model',
        ['NodeType'] = 'Model',
        ['Template'] = 'Model',
        ['Link'] = '$$.unit.hero001.Model_2',
        ['user_data'] = user_data['$$.unit.hero001'],
        ['event'] = event_register
    },
    ['root'] = {
        ['ActorArray'] = {
            [1] = '$$.unit.hero001.ActorModel_1',
        },
        ['AnimMapping'] = {
        },
        ['AttackableRadius'] = 80,
        ['Attribute'] = {
            ['半月斩等级'] = nil,
            ['暴风雪等级'] = nil,
            ['暴击'] = 10,
            ['暴击伤害'] = 2,
            ['暴怒秘籍'] = nil,
            ['爆燃秘籍'] = nil,
            ['辟地秘籍'] = nil,
            ['冰暴等级'] = nil,
            ['冰暴伤害秘籍'] = nil,
            ['冰风暴秘籍'] = 0,
            ['冰冷之心天赋等级'] = nil,
            ['冰霜召唤师秘籍'] = nil,
            ['不灭之躯天赋等级'] = nil,
            ['不屈意志天赋等级'] = nil,
            ['冲动天赋等级'] = nil,
            ['穿肠秘籍'] = nil,
            ['穿透'] = 1,
            ['出血秘籍'] = nil,
            ['脆化冻结'] = nil,
            ['单位标记'] = 0,
            ['等级'] = nil,
            ['癫火秘籍'] = nil,
            ['动脉打击天赋等级'] = nil,
            ['法术暴击'] = nil,
            ['法术暴击伤害'] = nil,
            ['法术穿透'] = nil,
            ['法术破甲'] = 15,
            ['焚天秘籍'] = 0,
            ['格挡'] = nil,
            ['攻击'] = 33,
            ['攻击范围'] = 500,
            ['攻击速度'] = 0.1,
            ['鼓舞光环等级'] = nil,
            ['寒冰箭等级'] = nil,
            ['寒冰增效天赋等级'] = nil,
            ['寒冰掌握秘籍'] = nil,
            ['护盾'] = nil,
            ['护甲'] = nil,
            ['火球术等级'] = nil,
            ['火球增效秘籍'] = nil,
            ['火焰亲和天赋等级'] = nil,
            ['火焰召唤师秘籍'] = nil,
            ['火焰之墙等级'] = nil,
            ['减耗'] = 1,
            ['剑刃乱舞等级'] = nil,
            ['减伤'] = nil,
            ['技能点'] = nil,
            ['经验'] = nil,
            ['经验上限'] = nil,
            ['急速'] = nil,
            ['亢奋秘籍'] = nil,
            ['快速冻结秘籍'] = nil,
            ['狂怒秘籍'] = nil,
            ['狂热秘籍'] = nil,
            ['冷却缩减'] = nil,
            ['裂地斩等级'] = nil,
            ['裂地震荡秘籍'] = nil,
            ['凛冬秘籍'] = nil,
            ['乱舞震荡秘籍'] = nil,
            ['命源秘籍'] = nil,
            ['魔法'] = nil,
            ['魔法恢复'] = nil,
            ['魔法上限'] = nil,
            ['魔抗'] = 0,
            ['怒气爆发等级'] = nil,
            ['破甲'] = 10,
            ['强化寒冰箭天赋等级'] = nil,
            ['强化火球术天赋等级'] = nil,
            ['强化灼烧天赋等级'] = nil,
            ['强击光环等级'] = nil,
            ['闪避'] = 0,
            ['闪烁能量秘籍'] = nil,
            ['生命'] = 1000,
            ['生命恢复'] = nil,
            ['生命上限'] = 1000,
            ['神力秘籍'] = nil,
            ['嗜血秘籍'] = nil,
            ['视野范围'] = nil,
            ['守护光环等级'] = nil,
            ['撕裂秘籍'] = nil,
            ['搜敌范围'] = 300,
            ['碎冰秘籍'] = nil,
            ['碎魂秘籍'] = nil,
            ['武力压制天赋等级'] = nil,
            ['吸血'] = 0,
            ['旋风斩等级'] = nil,
            ['雪崩秘籍'] = nil,
            ['炎帝等级'] = nil,
            ['移动速度'] = 600,
            ['月光斩天赋等级'] = nil,
            ['月刃秘籍'] = nil,
            ['召唤火焰卫士等级'] = nil,
            ['召唤水元素等级'] = nil,
            ['炙烤秘籍'] = nil,
            ['致命打击天赋等级'] = nil,
            ['致死秘籍'] = nil,
            ['转身速度'] = 500,
            ['灼烧等级'] = nil,
            ['逐日剑法天赋等级'] = nil,
            ['逐日斩等级'] = nil,
            ['逐日震荡秘籍'] = nil,
            ['最大转身速度'] = 1,
            ['最小转身速度'] = -1,
        },
        ['CustomUnitData'] = {
        },
        ['DefaultAI'] = '',
        ['Description'] = '',
        ['Effect'] = '',
        ['Filter'] = {
            [1] = '单位',
            [2] = '英雄',
        },
        ['Icon'] = '',
        ['Inventorys'] = {
            [1] = '$$.unit.hero001.ItemContainer_1',
        },
        ['ModelData'] = '$$.unit.hero001.Model_1',
        ['ModelScaleFactor'] = 1,
        ['Name'] = 'hero001',
        ['PackageInfo'] = 'characters/General/SK_General/model.prefab',
        ['PlanerShadow'] = true,
        ['Responses'] = {
        },
        ['Restrictions'] = {
        },
        ['ReviveTime'] = -1,
        ['ScanIgnoreVisibility'] = false,
        ['UnitData'] = {
            ['AttackSkill'] = nil,
            ['AttackableRadius'] = 60,
            ['AttackedHeight'] = 130,
            ['Attribute'] = {
                ['半月斩等级'] = nil,
                ['暴风雪等级'] = nil,
                ['暴击'] = nil,
                ['暴击伤害'] = nil,
                ['暴怒秘籍'] = nil,
                ['爆燃秘籍'] = nil,
                ['辟地秘籍'] = nil,
                ['冰暴等级'] = nil,
                ['冰暴伤害秘籍'] = nil,
                ['冰风暴秘籍'] = nil,
                ['冰冷之心天赋等级'] = nil,
                ['冰霜召唤师秘籍'] = nil,
                ['不灭之躯天赋等级'] = nil,
                ['不屈意志天赋等级'] = nil,
                ['冲动天赋等级'] = nil,
                ['穿肠秘籍'] = nil,
                ['穿透'] = nil,
                ['出血秘籍'] = nil,
                ['脆化冻结'] = nil,
                ['单位标记'] = nil,
                ['等级'] = nil,
                ['癫火秘籍'] = nil,
                ['动脉打击天赋等级'] = nil,
                ['法术暴击'] = nil,
                ['法术暴击伤害'] = nil,
                ['法术穿透'] = nil,
                ['法术破甲'] = nil,
                ['焚天秘籍'] = nil,
                ['格挡'] = nil,
                ['攻击'] = 33,
                ['攻击范围'] = 500,
                ['攻击速度'] = 1,
                ['鼓舞光环等级'] = nil,
                ['寒冰箭等级'] = nil,
                ['寒冰增效天赋等级'] = nil,
                ['寒冰掌握秘籍'] = nil,
                ['护盾'] = nil,
                ['护甲'] = nil,
                ['火球术等级'] = nil,
                ['火球增效秘籍'] = nil,
                ['火焰亲和天赋等级'] = nil,
                ['火焰召唤师秘籍'] = nil,
                ['火焰之墙等级'] = nil,
                ['减耗'] = nil,
                ['剑刃乱舞等级'] = nil,
                ['减伤'] = nil,
                ['技能点'] = nil,
                ['经验'] = nil,
                ['经验上限'] = nil,
                ['急速'] = nil,
                ['亢奋秘籍'] = nil,
                ['快速冻结秘籍'] = nil,
                ['狂怒秘籍'] = nil,
                ['狂热秘籍'] = nil,
                ['冷却缩减'] = nil,
                ['裂地斩等级'] = nil,
                ['裂地震荡秘籍'] = nil,
                ['凛冬秘籍'] = nil,
                ['乱舞震荡秘籍'] = nil,
                ['命源秘籍'] = nil,
                ['魔法'] = nil,
                ['魔法恢复'] = nil,
                ['魔法上限'] = nil,
                ['魔抗'] = nil,
                ['怒气爆发等级'] = nil,
                ['破甲'] = nil,
                ['强化寒冰箭天赋等级'] = nil,
                ['强化火球术天赋等级'] = nil,
                ['强化灼烧天赋等级'] = nil,
                ['强击光环等级'] = nil,
                ['闪避'] = nil,
                ['闪烁能量秘籍'] = nil,
                ['生命'] = 1000,
                ['生命恢复'] = nil,
                ['生命上限'] = 1000,
                ['神力秘籍'] = nil,
                ['嗜血秘籍'] = nil,
                ['视野范围'] = nil,
                ['守护光环等级'] = nil,
                ['撕裂秘籍'] = nil,
                ['搜敌范围'] = 300,
                ['碎冰秘籍'] = nil,
                ['碎魂秘籍'] = nil,
                ['武力压制天赋等级'] = nil,
                ['吸血'] = nil,
                ['旋风斩等级'] = nil,
                ['雪崩秘籍'] = nil,
                ['炎帝等级'] = nil,
                ['移动速度'] = 500,
                ['月光斩天赋等级'] = nil,
                ['月刃秘籍'] = nil,
                ['召唤火焰卫士等级'] = nil,
                ['召唤水元素等级'] = nil,
                ['炙烤秘籍'] = nil,
                ['致命打击天赋等级'] = nil,
                ['致死秘籍'] = nil,
                ['转身速度'] = 1800,
                ['灼烧等级'] = nil,
                ['逐日剑法天赋等级'] = nil,
                ['逐日斩等级'] = nil,
                ['逐日震荡秘籍'] = nil,
                ['最大转身速度'] = -1,
                ['最小转身速度'] = -1,
            },
            ['Block'] = {
                ['Center'] = {
                    ['X'] = 0.5,
                    ['Y'] = 0.5,
                },
                ['Footpoint'] = '',
                ['Size'] = nil,
            },
            ['BloodBarType'] = 'BLOODBAR_TYPE_HERO',
            ['BloodHeightScale'] = 1,
            ['BloodWidthScale'] = 1,
            ['BornEffect'] = '',
            ['ClientSelectFoot'] = 0,
            ['ClientSelectHead'] = 150,
            ['ClientSelectRadius'] = 70,
            ['ClientSelectable'] = true,
            ['CollisionRadius'] = 48,
            ['CollisionType'] = 1,
            ['DeadSink'] = 1,
            ['DeadSinkHeight'] = 300,
            ['DeathDelayTime'] = 0,
            ['DeathEffect'] = '',
            ['DeathEffectByEnemy'] = '',
            ['DeathEffectSpeed'] = 1,
            ['DeathShowTime'] = 0,
            ['DynamicPointLightShaderingEnable'] = true,
            ['Effect'] = '',
            ['EffectByEnemy'] = '',
            ['HeroSkill'] = {
                [1] = '$$.spell.火球术001.root',
                [2] = '$$default_units.spell.闪现.root',
                [3] = '',
                [4] = '',
            },
            ['HideSkill'] = nil,
            ['KillOnDeactivate'] = 0,
            ['LevelUpEffect'] = '',
            ['MaxTurnSpeed'] = -1,
            ['MinTurnSpeed'] = -1,
            ['MiniMapIcon'] = '',
            ['MiniMapIconEnemy'] = '',
            ['MiniMapIconWithFacing'] = false,
            ['MiniMapIconZindex'] = 1,
            ['ModelAttribute'] = {
                ['X轴缩放'] = nil,
                ['Y轴缩放'] = nil,
                ['Z轴缩放'] = nil,
                ['动画速度'] = nil,
                ['朝向'] = nil,
                ['缩放'] = 1.2,
            },
            ['ModelScaleFactor'] = 1,
            ['ModelType'] = 1,
            ['Module'] = {
                [1] = '运动',
                [2] = '移动',
                [3] = '不朽',
            },
            ['MoveAnimSpeedFactor'] = 0.714285,
            ['NeedOutScreenTick'] = 0,
            ['PackageInfo'] = 'characters/General/SK_General/model.prefab',
            ['ResourcePackageList'] = '',
            ['ResourceType'] = '魔法',
            ['Restriction'] = {
            },
            ['SelectedRadius'] = 80,
            ['ShowBlood'] = 1,
            ['Sync'] = 'sight',
            ['TurnSpeed'] = 1800,
            ['UnitClass'] = '英雄',
            ['UnitTag'] = '英雄',
            ['UpperBodyBone'] = '',
            ['Useable'] = 1,
            ['death_material'] = '',
        },
        ['UseRoleSH'] = false,
        ['distance_random'] = {
            [1] = 125,
            [2] = 175,
        },
        ['follow_random'] = 5,
        ['stay_time'] = 3,
        ['ID'] = 'root',
        ['Class'] = 'unit',
        ['NodeType'] = 'Unit',
        ['Template'] = 'Unit',
        ['Link'] = '$$.unit.hero001.root',
        ['user_data'] = user_data['$$.unit.hero001'],
        ['event'] = event_register
    },
}

return mt